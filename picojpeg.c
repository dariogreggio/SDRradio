...
// adapted by GK on 10-17 Oct 2021 (v. anche jpeg2vga.c)
// file reader 2022 GD

// per leggere file guardare: https://github.com/WilliamDidier/JPEG-reader/blob/master/src/jpeg_reader.c
...
static uint8_t processMarkers(uint8_t* pMarker) {
   ...
   for(;;)   {
      ...
      switch(c)      {
         ...
         case M_DAC:
            return PJPG_NO_ARITHMETIC_SUPPORT;
         case M_DQT:
            readDQTMarker();
            break;
         case M_DRI:
         ...
         case M_APP0:  /* FARE se si vuole (no need to read the JFIF marker) */
         case M_APP15:
					 break;
...
static void fixInBuffer(void) {
   ...
   if(gBitsLeft > 0)  
      stuffChar(LOBYTE(gBitBuf));
   
   stuffChar(HIBYTE(gBitBuf));
...
unsigned char pjpeg_decode_init(pjpeg_image_info_t *pInfo, pjpeg_need_bytes_callback_t pNeed_bytes_callback, void *pCallback_data, unsigned char reduce) {
	...
	pInfo->m_pMCUBufR = NULL; pInfo->m_pMCUBufG = NULL; pInfo->m_pMCUBufB = NULL;
